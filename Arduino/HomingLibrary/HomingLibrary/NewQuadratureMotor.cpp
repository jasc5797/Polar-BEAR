...
NewQuadratureMotor::NewQuadratureMotor(int pwmPin, int dirPin, int encoderPinA, int encoderPinB, int limitPin1, int limitPin2)
{
...
void NewQuadratureMotor::setState(STATE state)
{
...
void NewQuadratureMotor::moveOneLimit()
{
	moveHelper(limitSwitch1);
}

void NewQuadratureMotor::moveTwoLimit()
{
...
void NewQuadratureMotor::moveHelper(LimitSwitch* limitSwitch)
{
	int currentPosition = encoder->read();
...
void NewQuadratureMotor::setSpeed(int16_t speed)
{
	this->speed = speed;
	motor->setSpeed(speed);
}

bool NewQuadratureMotor::moveUntilPressed(LimitSwitch* limitSwitch, int16_t speed, int direction)
{
	limitSwitch->update();
	bool isPressed = limitSwitch->pressed();
	if (!isPressed)
...
bool NewQuadratureMotor::moveUntilReleased(LimitSwitch* limitSwitch, int16_t speed, int direction)
{	
	limitSwitch->update();
...
void NewQuadratureMotor::testTravelDistance(int count)
{
...
void NewQuadratureMotor::moveManual(bool alternate)
{
	...
	if (Serial.available())
	{
		...
		{
			isPressed = moveUntilPressed(limitSwitch, HOMING_SPEED, direction);
		} while (abs(startPos - encoder->read()) < 100 && !isPressed);
...
int NewQuadratureMotor::getTravelDistance()
{
	switch (limitMode)
	{
		case ONE:
...
int NewQuadratureMotor::getTravelDistanceOneLimit()
{
	bool isLimitPressed;
	do
	{
...
int NewQuadratureMotor::getTravelDistanceTwoLimit()
{
